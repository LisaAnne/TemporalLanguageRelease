...
while getopts ":HG:M:D:F:S:" opt; do
  case $opt in
    H) echo "Bash script to evaluate released models on tempoTL:"
       ...
       echo ""
       echo "   Flag 'M' (required): indicate which model to train"
       echo "       Options:"
...
if [[ -z $mode ]] ; then
    echo 'Must indicate which type of data to train on (rgb or flow)'
    exit 1
fi
...
echo "On input modality: " $mode

#Parameters shared across all models

if [ "$mode" == flow ]; then
    data=data/average_flow_feats.h5
elif [ "$mode" == rgb ]; then
    data=data/average_rgb_feats.h5
else
